namespace DeadMosquito.GoogleMapsView
{
	/// <summary>
	/// Cap that is a semicircle with radius equal to half the stroke width, centered at the start or end vertex of a <see cref="Polyline"/> with solid stroke pattern.
	/// 
	/// See: https://developers.google.com/android/reference/com/google/android/gms/maps/model/RoundCap
	/// </summary>
	public sealed class RoundCap : Cap
	{
	}
}